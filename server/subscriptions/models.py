...
class SubscriptionCoupon(models.Model):
    ...
    min_price = models.IntegerField(verbose_name="Min Subscription Price", default=0)                   # Min Subscription Price
    max_price = models.IntegerField(verbose_name="Max Subscription Price", default=1000000000)          # Max Subscription Price
    min_month = models.IntegerField(verbose_name="Min Month", default=1)                                # Min Month
    max_month = models.IntegerField(verbose_name="Max Month", default=12)                               # Max Month
    ...
    def generate_code(self):
        ...
        while True:
            ...
            if not SubscriptionCoupon.objects.filter(code=code).exists():
                return code
...
class SubscriptionUserCoupon(models.Model):
    user = models.ForeignKey(User, on_delete=models.CASCADE, related_name='subscription_user_coupons')
    coupon = models.ForeignKey(SubscriptionCoupon, on_delete=models.CASCADE, related_name='subscription_user_coupons')
    ...
    def clean(self):
        ...
        if self.pk is None:  # 새로 생성되는 경우에만
            if SubscriptionUserCoupon.objects.filter(user=self.user, coupon=self.coupon, used_at=None).exists():
                raise ValidationError("이미 해당 쿠폰을 보유하고 있습니다.")
        
        # 쿠폰의 전체 사용 제한 확인 (현재 레코드 제외)
        used_count = SubscriptionUserCoupon.objects.filter(
            coupon=self.coupon,
        ...
        user_used_count = SubscriptionUserCoupon.objects.filter(
            user=self.user,
            coupon=self.coupon,
        ).exclude(pk=self.pk).count()
...
class SubscriptionRequest(models.Model):
    ...
    car = models.ForeignKey(Car, on_delete=models.CASCADE, related_name='subscription_requests')
    ...
    def save(self, *args, **kwargs):
        ...
        if self.start_date and self.month:
            self.end_date = self.start_date + timedelta(days=30 * self.month)

        super().save(*args, **kwargs)

    def __str__(self):
        return f"{self.user.name} - {self.car.model.brand.name} {self.car.model.name} ({self.start_date} ~ {self.end_date})"
...
class SubscriptionLike(models.Model):
    id = models.AutoField(primary_key=True)
    model = models.ForeignKey(Model, on_delete=models.CASCADE, related_name='subscription_likes')
    user = models.ForeignKey(User, on_delete=models.CASCADE, related_name='subscription_likes')
...
class SubscriptionReview(models.Model):
    id = models.AutoField(primary_key=True)
    model = models.ForeignKey(Model, on_delete=models.CASCADE, related_name='subscription_reviews')
    user = models.ForeignKey(User, on_delete=models.CASCADE, related_name='subscription_reviews')
...
class SubscriptionReviewLike(models.Model):
    id = models.AutoField(primary_key=True)
    review = models.ForeignKey(SubscriptionReview, on_delete=models.CASCADE, related_name='subscription_review_likes')
    user = models.ForeignKey(User, on_delete=models.CASCADE, related_name='subscription_review_likes')
...
class SubscriptionModelRequest(models.Model):
    id = models.AutoField(primary_key=True)
    user = models.ForeignKey(User, on_delete=models.CASCADE, related_name='subscription_model_requests')
